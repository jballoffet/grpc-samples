...
class CachingInterceptor : public grpc::experimental::Interceptor {
 public:
  ...
  void Intercept(
    ...
    if (methods->QueryInterceptionHookPoint(
      ...
      stub_ = keyvalue_store::KeyValueStore::NewStub(
    ...
    if (methods->QueryInterceptionHookPoint(
      ...
      const keyvalue_store::Request* req_msg =
          static_cast<const keyvalue_store::Request*>(methods->GetSendMessage());
      if (req_msg != nullptr) {
        requested_key = req_msg->key();
      } else {
        // The non-serialized form would not be available in certain scenarios,
        // so add a fallback.
        keyvalue_store::Request req_msg;
        auto* buffer = methods->GetSerializedSendMessage();
        auto copied_buffer = *buffer;
        GPR_ASSERT(
            grpc::SerializationTraits<keyvalue_store::Request>::Deserialize(
      ...
      if (search != cached_map_.end()) {
        ...
        keyvalue_store::Request req;
        req.set_key(requested_key);
        stream_->Write(req);
        keyvalue_store::Response resp;
        stream_->Read(&resp);
    ...
    if (methods->QueryInterceptionHookPoint(
      ...
      keyvalue_store::Response* resp =
          static_cast<keyvalue_store::Response*>(methods->GetRecvMessage());
 ...
 private:
  ...
  std::unique_ptr<keyvalue_store::KeyValueStore::Stub> stub_;
  std::unique_ptr<
      grpc::ClientReaderWriter<keyvalue_store::Request, keyvalue_store::Response>>
      stream_;
